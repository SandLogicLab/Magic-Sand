...
namespace
{
	...
	const float DIG_DROP_THRESHOLD = 1.0f;
	const int DIG_CONFIRM_FRAMES = 1;
	const float DIG_COOLDOWN_SECONDS = 0.40f;
	const float DIG_MIN_TIME_BETWEEN_SAME_CELL = 0.15f;
...
void CTreasureSweeperController::drawProjectorWindow()
{
	...
	auto drawTextAllSides = [&](const std::string& msg, float scale, const ofColor& col, float yOffsetNorm)
	{
		ofSetColor(col);
		for (int i = 0; i < 4; ++i)
		{
	...
	if (roundState == ROUND_WIN)
	{
		...
		drawTextAllSides("YOU WIN!", 9.0f, ofColor(255, 250, 180, 245), -0.10f);
		drawTextAllSides("Next round: " + ofToString(nextRoundCountdown), 4.0f, ofColor(255, 240, 170, 245), 0.16f);
	}
	else if (roundState == ROUND_LOSE)
	{
		...
		drawTextAllSides("YOU LOSE", 8.5f, ofColor(255, 120, 120, 245), -0.10f);
		drawTextAllSides(loseHeadline, 4.5f, ofColor(255, 170, 170, 245), 0.10f);
		drawTextAllSides("Next round: " + ofToString(nextRoundCountdown), 3.7f, ofColor(255, 200, 200, 245), 0.24f);
	}
	else
	{
		ofSetColor(230);
