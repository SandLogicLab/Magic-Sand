...
namespace
{
	...
	const float DIG_DROP_THRESHOLD = 4.5f;
	...
	const float REVEAL_SECONDS_LOSE = 5.0f;
	const float REVEAL_SECONDS_WIN = 5.0f;
...
void CTreasureSweeperController::drawProjectorWindow()
{
	...
	if (roundState == ROUND_WIN)
	{
		...
		ofDrawBitmapString("*** TREASURE FOUND! ***", projRes.x * 0.40f, projRes.y * 0.46f);
		ofDrawBitmapString("YOU WIN!", projRes.x * 0.47f, projRes.y * 0.51f);
		ofDrawBitmapString("Next round in: " + ofToString(nextRoundCountdown), projRes.x * 0.43f, projRes.y * 0.56f);
	}
	else if (roundState == ROUND_LOSE)
	{
		...
		ofDrawBitmapString("!!! TRY AGAIN !!!", projRes.x * 0.44f, projRes.y * 0.44f);
		ofDrawBitmapString(loseHeadline, projRes.x * 0.42f, projRes.y * 0.50f);
		ofDrawBitmapString("Next round in: " + ofToString(nextRoundCountdown), projRes.x * 0.43f, projRes.y * 0.56f);
	}
	else
	{
